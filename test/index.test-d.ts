...
// spread resource with all columns in select query
{
  ...
  expectType<
    Prettify<
      { message: string | null } & Database["public"]["Tables"]["users"]["Row"]
    >
  >(
    data,
  );
...
{
  const { data: message, error } = await postgrest.from("messages").select(
    "user:users(*)",
  ).single();
  if (error) {
    throw new Error(error.message);
...
{
  const { data: user, error } = await postgrest.from("users").select(
    "messages(*)",
  ).single();
  if (error) {
    throw new Error(error.message);
...
// referencing missing column
{
  ...
  expectType<
    PostgrestSingleResponse<
      SelectQueryError<`Referencing missing column \`dat\``>[]
    >
  >(res);
}
...
// !left oneToOne
{
  const { data: oneToOne, error } = await postgrest.from("channel_details")
    .select(
      "channels!left(*)",
    ).single();
  ...
  expectType<Database["public"]["Tables"]["channels"]["Row"]>(
    oneToOne.channels,
  );
}
...
// !left oneToMany
{
  const { data: oneToMany, error } = await postgrest.from("users").select(
    "messages!left(*)",
  ).single();
  ...
  expectType<Array<Database["public"]["Tables"]["messages"]["Row"]>>(
    oneToMany.messages,
  );
}
...
// !left zeroToOne
{
  const { data: zeroToOne, error } = await postgrest.from("users").select(
    "user_profiles!left(*)",
  ).single();
  ...
  expectType<Array<Database["public"]["Tables"]["user_profiles"]["Row"]>>(
    zeroToOne.user_profiles,
  );
}
...
// join over a 1-1 relation with both nullables and non-nullables fields
{
  const { data: bestFriends, error } = await postgrest
    .from("best_friends")
    .select(
      "first_user(*), second_user(*), third_wheel(*)",
    ).single();
  ...
  expectType<Database["public"]["Tables"]["users"]["Row"]>(
    bestFriends.first_user,
  );
  expectType<Database["public"]["Tables"]["users"]["Row"]>(
    bestFriends.second_user,
  );
  // The third wheel should be optional
  expectType<Database["public"]["Tables"]["users"]["Row"] | null>(
    bestFriends.third_wheel,
  );
}
